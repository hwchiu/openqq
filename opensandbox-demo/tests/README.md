...
unique temporary tenant, verifies health and tenant-labelled metrics, checks that
unauthenticated traffic is rejected, confirms `/v1/snapshots` is not exposed, then
disables the temporary tenant. It does not create a sandbox or alter the warm pool.

Run from a cluster node:

```bash
CHECK_K8S=1 TENANT_SERVER_URL=http://127.0.0.1:18080 ./tenant-server-smoke.sh
```
